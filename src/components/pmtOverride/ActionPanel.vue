...
// 设置Toast回调
pythonValidator.setToastCallback((message) => {
  toast.add(message)
})
...
function resetAll() {
  pythonValidator.resetState()
  updateLocalState()
}

function updateLocalState() {
  const data = pythonValidator.getValidateData()
  typesFile.value = data.typesFile
  optionsFile.value = data.optionsFile
  pythonFile.value = data.pythonFile
  typesParsed.value = data.typesParsed
  optionsParsed.value = data.optionsParsed
  jsonGenerated.value = data.jsonGenerated
  jsonConfig.value = data.jsonConfig
  parsingTypes.value = data.parsingTypes
  parsingOptions.value = data.parsingOptions
  validating.value = data.validating
  typesParseError.value = data.typesParseError
  optionsParseError.value = data.optionsParseError
  pythonParseError.value = data.pythonParseError
}

function resetTypeFile() {
  pythonValidator.resetTypeFile()
  updateLocalState()
...
function resetOptionsFile() {
  pythonValidator.resetOptionsFile()
  updateLocalState()
...
function resetPythonFile() {
  pythonValidator.resetPythonFile()
  updateLocalState()
...
async function handleTypesFileSelect(event) {
  ...
  const success = await pythonValidator.handleTypesFileUpload(file)
  updateLocalState()

  if (!success && typesFileInputRef.value) {
    typesFileInputRef.value.value = ''
  }
}
...
async function handleOptionsFileSelect(event) {
  ...
  const success = await pythonValidator.handleOptionsFileUpload(file)
  updateLocalState()

  if (!success && optionsFileInputRef.value) {
    optionsFileInputRef.value.value = ''
  }
}
...
async function handlePythonFileSelect(event) {
  ...
  const success = await pythonValidator.handlePythonFileUpload(file)
  updateLocalState()

  if (!success && pythonFileInputRef.value) {
    pythonFileInputRef.value.value = ''
  }
}

function exportJsonConfig() {
  pythonValidator.exportJsonConfig()
  updateLocalState()
}
    ...
    if (file.type === 'application/json' || file.name?.endsWith('.json')) {
      ...
      reader.onload = async () => {
        ...
        if (jsonContent?.plugin_name) {
          // pmt plugin config
          try {
            const defs = pythonValidator.pluginConfig2ComfyNodeDefs(
              jsonContent,
              false
            )
